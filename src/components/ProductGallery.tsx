...
export default function ProductGallery() {
  return (
    <article data-gsap-hover className="rounded-[34px] border border-[#a9bfd0] bg-[#f1f9ff] p-4 sm:rounded-[24px] sm:border-0 sm:bg-[#e8f0f6]">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2.5 sm:gap-3">
        {walkieItems.map((item) => (
          <article key={item.title} data-gsap-hover className="overflow-hidden rounded-[24px] bg-white/10 sm:rounded-[18px]">
            <Image
              src={item.image}
              alt={item.title}
              width={327}
              height={278}
              className="gsap-hover-image h-[182px] w-full object-cover sm:h-[178px]"
            />
